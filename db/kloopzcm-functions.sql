...
-- DROP FUNCTION dj_create_rfc_ci(bigint, bigint, bigint, bigint, integer, character varying, character varying, integer, integer, bigint, character varying, character varying);
...
-- DROP FUNCTION dj_create_rfc_relation(bigint, bigint, bigint, bigint, bigint, bigint, integer, character varying, bigint, bigint, integer, integer, bigint, character varying, character varying);

CREATE OR REPLACE FUNCTION dj_create_rfc_relation(p_rfc_id bigint, p_release_id bigint, p_ns_id bigint, p_ci_relation_id bigint, p_from_rfc_id bigint, p_from_ci_id bigint, p_relation_id integer, p_relation_goid character varying, p_to_rfc_id bigint, p_to_ci_id bigint, p_action_id integer, p_exec_order integer, p_last_rfc_id bigint, p_comments character varying, p_created_by character varying)
  RETURNS void AS
$BODY$
BEGIN
...
-- DROP FUNCTION dj_upsert_rfc_ci_attr(bigint, integer, text, character varying, character varying);

CREATE OR REPLACE FUNCTION dj_upsert_rfc_ci_attr(IN p_rfc_id bigint, IN p_attribute_id integer, IN p_new_attr_value text, IN p_owner character varying, IN p_comments character varying, OUT p_rfc_attr_id bigint)
  RETURNS bigint AS
$BODY$
DECLARE
...
BEGIN
	...
	if not FOUND then			
		...
		where cia.ci_id = rfci.ci_id
		  ...
		  and cia.attribute_id = p_attribute_id;
...
-- DROP FUNCTION dj_upsert_rfc_rel_attr(bigint, integer, text, character varying, character varying);

CREATE OR REPLACE FUNCTION dj_upsert_rfc_rel_attr(IN p_rfc_id bigint, IN p_attribute_id integer, IN p_new_attr_value text, IN p_owner character varying, IN p_comments character varying, OUT p_rfc_attr_id bigint)
  RETURNS bigint AS
$BODY$
DECLARE
    l_old_attr_value text;
...
BEGIN
   ...
   update dj_rfc_ci
        ...
        INSERT INTO cms_ci_event_queue(event_id, source_pk, source_name, event_type_id)
                VALUES (nextval('event_pk_seq'), p_rfc_id, 'rfc_ci' , 200);
END;
$BODY$
  LANGUAGE plpgsql VOLATILE
  COST 100;
